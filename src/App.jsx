...
function transformOMToDays(data) {
  ...
  for (let i = 0; i < len; i++) {
    ...
    const rh = rhArr.length
      ? Math.round(rhArr.reduce((a, b) => a + b, 0) / rhArr.length)
      : 60;
...
function sliceHourlyForDay(hourly, dayIndex) {
  ...
  const d0 =
    hourly.time.find((t) => t.includes("T00:00"))?.slice(0, 10) ||
  ...
  return {
    hours,
    tempF: idxs.map((i) => hourly.tempF[i]),
    rainProb: idxs.map((i) => hourly.rainProb[i]),
  };
}
...
function autoflowerOutdoorKnowledgePack(startDate) {
  ...
  return [
    W(1, "Germinate & establish seedling", [
      "Start in Rapid Rooter; 75–80°F",
      "Lightly moist; gentle airflow",
      "Transplant when roots show or day 10",
    ]),
    W(2, "Transplant to final pot (3–5 gal)", [
      "Plant plug gently",
      "Begin light nutrients (¼–½ strength)",
    ]),
    W(3, "Early veg care", [
      "Check pests; one preventative spray (no buds)",
      "Stake if windy; 6–8h direct sun",
    ]),
    W(4, "Pre-flower checks", [
      "Topdress/adjust feed; watch stretch",
      "Light defol only if airflow blocked",
    ]),
    W(5, "Early flower support", [
      "Support branches; avoid wet buds",
      "Begin bloom nutrients gradually",
    ]),
    W(6, "Flower maintenance", [
      "Inspect daily for mold/caterpillars",
      "Water to light runoff; avoid overwatering",
    ]),
    W(7, "Rain & humidity vigilance", [
      "Prepare rain shield",
      "Increase airflow around colas",
    ]),
    W(8, "Late flower care", [
      "Reduce N; maintain P/K",
      "Check trichomes (cloudy→amber)",
    ]),
    W(9, "Ripening & flush (optional)", [
      "If flushing, start 7–10 days before harvest",
      "Minimal handling of wet buds",
    ]),
    W(10, "Harvest & dry setup", [
      "Harvest at cloudy/some amber",
      "Dry 60–70°F, 50–60% RH; gentle airflow",
    ]),
  ];
}
...
function evaluateWeatherRules(grow, simWeather /*, timeline */) {
  ...
  if ((stage === "flower_early" || stage === "flower_late") && (avgRH >= 85 || maxRain >= 70)) {
    alerts.push({
      id: uid(),
      severity: "high",
      title: "High humidity / rain incoming",
      message: "Deploy rain shield, keep buds dry, vent mid-day.",
    });
  }
  if (minLow <= 40)
    alerts.push({
      id: uid(),
      severity: "high",
      title: "Possible frost risk",
      message: "Cover at night or move pots inside; remove cover after sunrise.",
    });
  else if (minLow <= 48)
    alerts.push({
      id: uid(),
      severity: "medium",
      title: "Chilly nights ahead",
      message: "Consider evening cover; water earlier to reduce night RH.",
    });
  if (maxHigh >= 92)
    alerts.push({
      id: uid(),
      ...
      message: "Provide midday shade; ensure soil doesn’t bone-dry.",
    });
  return alerts;
}
...
const TempLineChart = ({ hours, values }) => {
  ...
  return (
    <svg viewBox={`0 0 ${w} ${h}`}>
      ...
      {values.map((v, i) => (
        <circle key={i} cx={xi(i)} cy={yi(v)} r="2" fill="#2a6" />
      ))}
      <text x={pad} y={h - 6} fontSize="10" fill="#666">
        {hours?.[0]}
      </text>
      <text x={w - pad - 20} y={h - 6} fontSize="10" fill="#666">
...
export default function App() {
  ...
  const activeGrow = useMemo(
    () => grows.find((g) => g.id === activeGrowId) || null,
    [grows, activeGrowId]
  );
  const timeline = useMemo(
    () => (activeGrow ? generateAutoflowerOutdoorTimeline(activeGrow) : []),
    [activeGrow]
  );
  const alerts = useMemo(
    () => (activeGrow ? evaluateWeatherRules(activeGrow, simWeather) : []),
    [activeGrow, simWeather]
  );
  ...
  return (
    <>
      <header className="top-header">
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <svg width="22" height="22" viewBox="0 0 24 24" fill="none">
            <path
              d="M4 13c0 4 3 7 7 7 6 0 9-6 9-12V6h-2C10 6 4 8 4 13Z"
              stroke="#2a6"
              strokeWidth="2"
              fill="#dff6e8"
            ></path>
            <path d="M8 17c1-3 5-6 8-7" stroke="#2a6" strokeWidth="2"></path>
          </svg>
          <h1 style={{ margin: 0, fontSize: 20 }}>Autoflower Grow Assistant — No-AI</h1>
        </div>
        <div style={{ opacity: 0.7, fontSize: 12 }}>
          Static build • Offline-first • Open-Meteo
        </div>
      </header>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {[...grows]
              ...
              .map((g) => (
                <div
                  key={g.id}
                  className={`grow-item ${g.id === activeGrowId ? "active" : ""}`}
                  style={{ opacity: g.status === "archived" ? 0.6 : 1 }}
                >
                  <button
                    style={{
                      textAlign: "left",
                  ...
                  >
                    ...
                    <div style={{ fontSize: 12, opacity: 0.7 }}>
                      {g.environment} • {g.style} • {fmtDate(g.startDate)}
                    </div>
                  </button>
                  <button
                    className="btn small"
                  ...
                  >
                    {g.status === "archived" ? "Unarchive" : "Archive"}
                  </button>
                </div>
              ))}
            {grows.length === 0 && <div style={{ opacity: 0.7 }}>No grows yet.</div>}
          </div>
        </aside>
          ...
          {showOnboarding ? (
            ...
            <>
              <GrowSummary
                grow={activeGrow}
                onUpdate={(patch) => {
                  setGrows((cur) => cur.map((g) => (g.id === patch.id ? { ...g, ...patch } : g)));
                }}
              />
                    ...
                    onToggleTask={(taskId) => {
                      setGrows((cur) =>
                        cur.map((g) => {
                          ...
                          const tasks = (g.tasks || timeline).map((t) =>
                            t.id === taskId ? { ...t, state: t.state === "done" ? "pending" : "done" } : t
                          );
                          return { ...g, tasks };
                        })
                      );
                ...
                <Card title="Journal">
                  <Journal
                    ...
                    onAdd={(e) => {
                      setJournal((cur) => ({
                        ...cur,
                        [activeGrow.id]: [e, ...(cur[activeGrow.id] || [])],
                      }));
                    }}
                    onDelete={(id) => {
                      setJournal((cur) => ({
                        ...cur,
                        [activeGrow.id]: (cur[activeGrow.id] || []).filter((x) => x.id !== id),
                      }));
                    }}
                  />
                </Card>
      ...
      <footer className="footer">
        <small>
          Tip: Click “Sync now” to fetch your local forecast. Data is cached in your browser.
        </small>
      </footer>
    </>
  );
  ...
  return (
    <div className="card" style={{ maxWidth: 820 }}>
      ...
      <div className="form-grid">
        ...
        <Labeled label="Location (free-text)">
          <input
            className="input"
            value={form.location}
        ...
        <Labeled label="Pot Size (gal)">
          <input
            type="number"
            min="1"
            className="input"
            value={form.potSizeGallons}
            onChange={(e) => setForm({ ...form, potSizeGallons: +e.target.value })}
          />
        </Labeled>
      </div>
      <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
        <button
          ...
          onClick={() => onCreate({ ...form, id: uid(), status: "active" })}
        >
          Create
        </button>
    ...
    <div className="summary" style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
      {!editing ? (
        <>
          <div>
            ...
            <div style={{ opacity: 0.8 }}>
              {grow.environment} • {grow.style} • {grow.medium} • {grow.potSizeGallons} gal
            </div>
            {grow.latitude !== undefined && grow.longitude !== undefined && (
              <div style={{ fontSize: 12, opacity: 0.7 }}>
                Lat {Number(grow.latitude).toFixed(4)}, Lon {Number(grow.longitude).toFixed(4)}
              </div>
            )}
          </div>
          <div style={{ textAlign: "right" }}>
            <div>
              <b>Start:</b> {fmtDate(grow.startDate)}
            </div>
            <div>
              <b>Target harvest:</b> {fmtDate(endDate)}
            </div>
            <div>
              <button className="btn small" style={{ marginTop: 6 }} onClick={() => setEditing(true)}>
                Edit details
              </button>
            </div>
          </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 8, width: "100%" }}>
          ...
          <Labeled label="Latitude">
            <input type="number" step="0.0001" className="input" value={draft.latitude ?? 38.7842} onChange={(e) => setDraft({ ...draft, latitude: +e.target.value })} />
          </Labeled>
          <Labeled label="Longitude">
            <input type="number" step="0.0001" className="input" value={draft.longitude ?? -77.5029} onChange={(e) => setDraft({ ...draft, longitude: +e.target.value })} />
          </Labeled>
          <Labeled label="Pot Size (gal)">
            <input type="number" min="1" className="input" value={draft.potSizeGallons} onChange={(e) => setDraft({ ...draft, potSizeGallons: +e.target.value })} />
          </Labeled>
          <div style={{ gridColumn: "1 / -1", display: "flex", gap: 8, justifyContent: "flex-end" }}>
            <button className="btn ghost" onClick={() => { setEditing(false); setDraft(grow); }}>
              Cancel
            </button>
            <button className="btn primary" onClick={() => { onUpdate({ ...draft, id: grow.id }); setEditing(false); }}>
              Save
            </button>
          </div>
        </div>
...
function TimelineView({ timeline, onToggleTask }) {
      ...
      {timeline.map((t) => (
        <div
          ...
          style={{
            ...
            background: t.state === "done" ? "#f1fff3" : undefined,
          }}
        >
          <div style={{ opacity: 0.8 }}>W{t.week}</div>
          <div>
          ...
          <div style={{ textAlign: "right" }}>
            <div style={{ fontSize: 12, opacity: 0.7 }}>{fmtDate(t.dueAt)}</div>
            <button className="btn small" onClick={() => onToggleTask(t.id)}>
              {t.state === "done" ? "Undo" : "Done"}
            </button>
            {new Date(t.dueAt) < new Date(today.toDateString()) && t.state !== "done" && (
              <div style={{ color: "#b30000", fontSize: 12 }}>Past due</div>
            )}
          </div>
        </div>
      ))}
      ...
      <div style={{ marginTop: 10 }}>
        {tab === "note" && (
          <div className="form-row">
            ...
            />
            <div>
              <button className="btn primary" onClick={() => add("note")} disabled={!form.text.trim()}>
                Add Note
              </button>
            </div>
          </div>
        ...
        {tab === "feeding" && (
          <div className="form-grid-3">
            <Labeled label="Volume (L)">
              <input className="input" value={form.volumeL} onChange={(e) => setForm({ ...form, volumeL: e.target.value })} />
            </Labeled>
            <Labeled label="pH">
              <input className="input" value={form.ph} onChange={(e) => setForm({ ...form, ph: e.target.value })} />
            </Labeled>
            <Labeled label="EC">
              <input className="input" value={form.ec} onChange={(e) => setForm({ ...form, ec: e.target.value })} />
            </Labeled>
            <div style={{ gridColumn: "1 / -1" }}>
              <button className="btn primary" onClick={() => add("feeding")} disabled={!form.volumeL}>
                Log Feeding
              </button>
            </div>
          </div>
        ...
        {tab === "measurement" && (
          <div className="form-grid-3">
            ...
            <Labeled label="Runoff pH">
              <input className="input" value={form.runoffPh} onChange={(e) => setForm({ ...form, runoffPh: e.target.value })} />
            </Labeled>
            <div style={{ gridColumn: "1 / -1" }}>
              <button
                className="btn primary"
                onClick={() => add("measurement")}
                disabled={!form.ph && !form.ec && !form.runoffPh}
              >
                Log Measurement
              </button>
            </div>
        ...
        {tab === "photo" && (
          <div className="form-grid">
            ...
            <div>
              <button className="btn primary" onClick={() => add("photo")} disabled={!form.photoUrl.trim()}>
                Add Photo
              </button>
            </div>
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {entries.map((e) => (
              <div key={e.id} className="entry">
                <div>
                  <div style={{ fontSize: 12, opacity: 0.7 }}>
                    {fmtDateTime(e.ts)} • {e.type}
                  </div>
                  {e.type === "note" && <div>{e.text}</div>}
                  {e.type === "feeding" && (
                    <div>
                      Fed {e.volumeL} L @ pH {e.ph ?? "–"} EC {e.ec ?? "–"} ({e.strength})
                    </div>
                  )}
                  {e.type === "measurement" && (
                    <div>
                      pH {e.ph ?? "–"}, EC {e.ec ?? "–"}, Runoff pH {e.runoffPh ?? "–"}
                    </div>
                  )}
                  {e.type === "photo" && e.url && (
                ...
                <button className="btn ghost" onClick={() => onDelete(e.id)}>
                  Delete
                </button>
              </div>
            ))}
  ...
  return (
    <div>
      <p style={{ marginTop: 0, opacity: 0.8 }}>
        Enter a simple 7-day forecast. Live API can overwrite this when you Sync.
      </p>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 8 }}>
        {days.map((d, idx) => (
          <div key={idx} className="weather-cell">
            <div style={{ fontWeight: 700 }}>D{idx + 1}</div>
            <Labeled label="Low (°F)" small>
              <input className="input" value={d.lowF} onChange={(e) => updateDay(idx, { lowF: +e.target.value })} />
            </Labeled>
            <Labeled label="High (°F)" small>
              <input className="input" value={d.highF} onChange={(e) => updateDay(idx, { highF: +e.target.value })} />
            </Labeled>
            <Labeled label="RH %" small>
              <input className="input" value={d.rh} onChange={(e) => updateDay(idx, { rh: +e.target.value })} />
            </Labeled>
            <Labeled label="Rain %" small>
              <input className="input" value={d.rainChance} onChange={(e) => updateDay(idx, { rainChance: +e.target.value })} />
            </Labeled>
          </div>
        ))}
      </div>
...
function WeatherLive({ grow, sim, setSim }) {
  const [status, setStatus] = useState({
    syncing: false,
    error: null,
    last: loadLS(`om_last_${grow.id}`, null),
  });
  ...
  const dayOptions = (sim.days || []).map((_, i) => ({
    label: i === 0 ? "Today" : `Day ${i + 1}`,
    index: i,
  }));
  const hourlyForDay = useMemo(
    () => (hourly ? sliceHourlyForDay(hourly, selectedDay) : null),
    [hourly, selectedDay]
  );
  ...
  return (
    <div>
      <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
        <button className="btn primary" onClick={syncNow} disabled={status.syncing}>
          {status.syncing ? "Syncing…" : "Sync now"}
        </button>
        {status.last && <div style={{ fontSize: 12, opacity: 0.7 }}>Last sync: {fmtDateTime(status.last)}</div>}
        ...
        <div className="card" style={{ padding: 12 }}>
          {sim.days?.[selectedDay] ? (
            <>
              ...
              <Row label="Low / High">
                {Math.round(sim.days[selectedDay].lowF)}° / {Math.round(sim.days[selectedDay].highF)}°F
              </Row>
              <Row label="Avg RH">{sim.days[selectedDay].rh}%</Row>
              <Row label="Rain chance">{sim.days[selectedDay].rainChance}%</Row>
              <div style={{ fontSize: 12, opacity: 0.7, marginTop: 8 }}>
                Use the selector to view the rest of the week.
              </div>
            </>
          ) : (
            <div style={{ opacity: 0.7 }}>No forecast yet. Click Sync.</div>
          )}
        </div>
        ...
        <div className="card" style={{ padding: 12 }}>
          {hourlyForDay ? (
            <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: 14 }}>
              <div>
                <div style={{ fontWeight: 700, marginBottom: 6 }}>Hourly temperature (°F)</div>
                <TempLineChart hours={hourlyForDay.hours} values={hourlyForDay.tempF} />
              </div>
              <div>
                <div style={{ fontWeight: 700, marginBottom: 6 }}>Hourly rain probability (%)</div>
                <RainBarChart hours={hourlyForDay.hours} values={hourlyForDay.rainProb} />
              </div>
            </div>
          ) : (
            <div style={{ opacity: 0.7 }}>Charts will appear after syncing weather.</div>
          )}
        </div>
      </div>
      ...
      <div style={{ marginTop: 10 }}>
        <details>
          <summary style={{ cursor: "pointer" }}>Manual edit (Weather Simulator)</summary>
          <div style={{ marginTop: 8 }}>
            <WeatherSim sim={sim} setSim={setSim} />
          </div>
        </details>
      </div>
    </div>
...
function AlertList({ alerts }) {
  if (!alerts || alerts.length === 0)
    return <div style={{ marginTop: 10, opacity: 0.7 }}>No weather-based alerts. You’re good for now.</div>;
    ...
    <div style={{ marginTop: 10, display: "flex", flexDirection: "column", gap: 8 }}>
      {alerts.map((a) => (
        <div key={a.id} className="alert" style={{ borderLeft: `4px solid ${color(a.severity)}` }}>
          ...
          <div style={{ fontSize: 13, opacity: 0.85 }}>{a.message}</div>
        </div>
      ))}
    </div>
  );
...
function DataIO({ grows, journal, simWeather, onImport }) {
  function exportJSON() {
    const blob = new Blob([JSON.stringify({ grows, journal, simWeather }, null, 2)], {
      type: "application/json",
    });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = `grow-assistant-${Date.now()}.json`;
    a.click();
  ...
  return (
    <div>
      ...
      <div style={{ fontSize: 12, opacity: 0.7, marginTop: 6 }}>
        Export includes grows, journal, and last forecast.
      </div>
    </div>
  );
}
